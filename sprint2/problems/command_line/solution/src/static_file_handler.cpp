...
http::response<http::string_body> StaticFileHandler::HandleRequest(
    ...
    auto make_plain_response = [&](http::status status, std::string body) {
    ...
    if (req.method() != http::verb::get && req.method() != http::verb::head) {
        return make_plain_response(http::status::method_not_allowed, "Invalid method");
    }
...
// Явная инстанциация для string_body, который у тебя сейчас используется в сервере
