...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req,
                    ...
                    Send&& send) const {
        const auto& game = application_.GetGame();
        ...
        if (target == "/api/v1/game/join") {
            ...
            try {
                ...
                const auto join_result = game.JoinPlayer(model::Map::Id(map_id), user_name);
                if (!join_result) {
        ...
        if (target == "/api/v1/game/players") {
            handle_authorized_get([&](std::string_view token) {
                ...
                json::object response;
                for (const auto& other_player : game.GetPlayersOnMap(player->map_id)) {
        ...
        if (target == "/api/v1/game/player/action") {
            ...
            try {
                ...
                if (!game.SetPlayerAction(*token, *move)) {
                    send_error(http::status::unauthorized,
                               "unknownToken",
                               "Player token has not been found");
                    return;
        ...
        if (target == "/api/v1/game/tick") {
            // Reject manual tick when server is in auto-tick mode
            if (application_.IsAutoTickMode()) {
                send_error(http::status::bad_request,
                           "badRequest",
                           "Invalid endpoint");
                return;
        ...
        if (target == "/api/v1/game/state") {
            handle_authorized_get([&](std::string_view token) {
                const auto game_state = game.GetStateByToken(token);
                if (!game_state) {
                    send_error(http::status::unauthorized,
                               "unknownToken",
                               "Player token has not been found");
                ...
                for (const auto& snap : game_state->players) {
                    json::array pos;
                ...
                for (const auto& loot : game_state->lost_objects) {
                    json::array pos;
                    pos.emplace_back(loot.pos.x);
                    pos.emplace_back(loot.pos.y);

                    json::object entry;
                    entry["type"] = loot.type;
                    entry["pos"] = std::move(pos);
                    lost_objects_obj[std::to_string(loot.id)] = std::move(entry);
                }
        ...
        if (target.rfind(std::string(maps_prefix), 0) == 0) {
            ...
            if (const auto* map = game.FindMap(model::Map::Id(map_id))) {
                auto map_json = MapToJson(*map);
                if (const auto* loot_types = extra_data_.GetMapLootTypes(*map->GetId())) {
                    map_json["lootTypes"] = *loot_types;
                } else {
                    map_json["lootTypes"] = json::array{};
                }
                send_json(http::status::ok, map_json);
                return;
            }
