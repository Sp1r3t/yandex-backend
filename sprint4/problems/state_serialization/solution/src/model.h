...
public:
    ...
    // Flat record for serialization/deserialization of a single player's state
    struct PlayerRecord {
        Player::Id id;
        std::string token;
        std::string map_id;
    ...
    std::vector<PlayerRecord> GetPlayersForSerialization() const;
    Player::Id GetNextPlayerId() const noexcept { return next_player_id_; }
    std::vector<std::pair<std::string, std::vector<LostObject>>> GetLostObjectsForSerialization() const;
    unsigned GetNextLostObjectId() const noexcept { return next_lost_object_id_; }
    void RestoreState(std::vector<PlayerRecord> players,
