...
Game::Game(Game&& other) noexcept {
    ...
    dog_retirement_time_ms_ = other.dog_retirement_time_ms_;
    ...
    retirement_handler_ = std::move(other.retirement_handler_);
}
...
void Game::Tick(std::int64_t time_delta_ms) const {
    std::vector<RetiredPlayerRecord> retirements;

    {
        std::lock_guard guard(players_mutex_);

        // Update play time and idle time (based on state at start of tick)
        for (Player& player : players_) {
            Dog& dog = player.GetDog();
            dog.AddPlayTime(time_delta_ms);
            if (!dog.IsMoving()) {
                dog.AddIdleTime(time_delta_ms);
            }
        }
        ...
        for (const auto& [map_id, player_indices] : map_players) {
            ...
            for (const auto& event : events) {
                ...
                if (event.item_id < loot_count) {
                    ...
                    // Office: score items and clear bag
                    if (dog.GetBagSize() > 0) {
                        int score = 0;
                        const auto& values = map->GetLootValues();
                        for (const auto& item : dog.GetBag()) {
                            if (item.type < values.size()) {
                                score += values[item.type];
                            } else {
                                score += 1;
                            }
                        }
        ...
        // Retire idle players
        if (dog_retirement_time_ms_ > 0) {
            std::vector<bool> retiring(players_.size(), false);
            for (size_t i = 0; i < players_.size(); ++i) {
                const Dog& dog = players_[i].GetDog();
                if (dog.GetIdleTime() >= dog_retirement_time_ms_) {
                    retiring[i] = true;
                    retirements.push_back({dog.GetName(), dog.GetScore(), dog.GetPlayTime()});
                }
            }
            ...
            if (!retirements.empty()) {
                ...
                for (const auto& [token, idx] : token_to_player_index_) {
                    if (retiring[idx]) tokens_to_remove.push_back(token);
                }
                for (const auto& t : tokens_to_remove) {
                    token_to_player_index_.erase(t);
                }

                // Update remaining token indices
                for (auto& [token, idx] : token_to_player_index_) {
                    idx = new_index[idx];
                }

                // Compact players vector
    ...
    } // release players_mutex_

    // Call retirement handler outside the lock
    if (retirement_handler_) {
        for (const auto& record : retirements) {
            retirement_handler_(record);
        }
    }
}
