...
int main(int argc, const char* argv[]) {
    ...
    try {
        ...
        auto db = std::make_shared<postgres::Database>(db_url, num_threads);

        game.SetRetirementHandler([db](const model::Game::RetiredPlayerRecord& record) {
            db->SaveRetiredPlayer(record.name, record.score, record.play_time_ms);
        });
        ...
        http_handler::RequestHandler handler{game, extra_data, args->www_root, db};
