...
public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req,
        ...
        // Records endpoint (no auth required)
        {
            ...
            if (path == "/api/v1/game/records") {
                ...
                auto parse_int_param = [&](const std::string& key, int& out) -> bool {
                    const std::string prefix = key + "=";
                    size_t pos = query.find(prefix);
                    if (pos == std::string::npos) return true;
                    pos += prefix.size();
                    size_t end = query.find('&', pos);
                    const std::string val = query.substr(pos, end == std::string::npos ? end : end - pos);
                    try {
                        out = std::stoi(val);
                    } catch (...) {
                        return false;
                ...
                if (!parse_int_param("start", start) || !parse_int_param("maxItems", max_items)) {
                    send_error(http::status::bad_request, "invalidArgument", "Invalid query parameter");
                    return;
                }
                ...
                try {
                    const auto records = db_->GetRetiredPlayers(start, max_items);
                    json::array arr;
