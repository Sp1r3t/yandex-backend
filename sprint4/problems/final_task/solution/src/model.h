...
public:
    ...
    int64_t GetIdleTime() const noexcept {
        return idle_time_ms_;
    }

    void AddIdleTime(int64_t ms) noexcept {
        idle_time_ms_ += ms;
    }
    ...
    void AddPlayTime(int64_t ms) noexcept {
        play_time_ms_ += ms;
    }
...
private:
    ...
    int score_ = 0;
    int64_t idle_time_ms_ = 0;
    int64_t play_time_ms_ = 0;
};
...
public:
    ...
    using RetirementHandler = std::function<void(const RetiredPlayerRecord&)>;
    ...
    void SetDogRetirementTime(int64_t retirement_time_ms) noexcept {
        dog_retirement_time_ms_ = retirement_time_ms;
    }

    void SetRetirementHandler(RetirementHandler handler) {
        retirement_handler_ = std::move(handler);
    }
...
private:
    ...
    RetirementHandler retirement_handler_;
};
